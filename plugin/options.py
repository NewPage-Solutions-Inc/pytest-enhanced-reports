...
def _get_value(request: FixtureRequest, arg_name: str):
    ...
    val_from_env_var = common_utils.get_env_var(full_arg_name.upper(), default_value=__args[arg_name]["default_value"])
...
def get_all_values(request: FixtureRequest):
    ...
    __arg_values = {arg_name: _get_value(request, arg_name) for arg_name in __args.keys()}
    return __arg_values
