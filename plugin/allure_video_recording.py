...
class ScreenRecorder:
    ...
    def create_video_from_images(self, scenario_info, location, video_size: tuple, frame_rate: int):
        ...
        for img in images_path:
            if img.__contains__('png'):
                video.write(cv2.resize(cv2.imread(os.path.join(self.directory, img)), video_size))
        video.release()

        logger.info("TEST EXECUTION VIDEO RECORDING VIDEO STOPPED [Video Size: " + str(video_size) + " - Frame Rate: "
                    + str(frame_rate) + "]")
    ...
    def stop_recording_and_stitch_video(self, video_info, recorder_thread, scenario_info, attachment_name):
        try:
            ...
            file_name = self.create_video_from_images(scenario_info, self.directory, video_resize_info,
            ...
            if video_info['keep_videos']:
                ...
                self.create_video_from_images(scenario_info, self.video_store, original_size,
                                              video_info['video_frame_rate'])
    ...
    def get_original_resolution(self, directory):
        ...
        img = Image.open(os.path.join(directory, [f for f in os.listdir(directory) if f.endswith('.png')][0]))
