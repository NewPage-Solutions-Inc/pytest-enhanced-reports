...
def _get_resized_image(image_bytes, scenario_name: str, options: dict, screenshot_name="screenshot.png"):
    ...
    if options['keep_screenshots']:
        common_utils._mkdir(f"{options['screenshot_dir']}/{scenario_name}")
        path: str = f"{options['screenshot_dir']}/{scenario_name}/{screenshot_file_name}"
        img.save(path)
    ...
    path: str = f"{options['screenshot_dir']}/{scenario_name}.png"
    img.save(path)
    return path


def highlight_element_and_take_a_screenshot(element, action_name, screen_shot_plugin_options, driver: WebDriver,
                                            color="red", border_width=5):
    def apply_style(s):
